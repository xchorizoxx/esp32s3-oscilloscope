"""
measurements_panel.py — Panel lateral/inferior de mediciones automáticas.
"""
...
class MeasurementsPanel(QDockWidget):
    ...
    def update_measurements(self, data: dict):
        """
        Actualiza la tabla con los datos del diccionario.
        Formato esperado:
        {'ch0': {'vpp_mv': 123.4, ...}, 'ch1': {...}}
        """
        ch0 = data.get('ch0')
        ch1 = data.get('ch1')
        
        if ch0: self._fill_col(0, ch0)
        if ch1: self._fill_col(1, ch1)
    ...
    def _fill_col(self, col: int, meas: dict):
        ...
        def fmt_v(v): return f"{v:.1f} mV" if abs(v) < 1000 else f"{v/1000:.2f} V"
        def fmt_f(f): return f"{f:.1f} Hz" if f < 1000 else f"{f/1000:.2f} kHz"
        def fmt_t(t): return f"{t:.1f} µs" if t < 1000 else f"{t/1000:.2f} ms"
